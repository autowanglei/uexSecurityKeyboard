...
public class RC4Encryption {
    public static final String F_KEY = "appcanuse$%^&%^&%^&%&^$#$#sdfsda90239%^)f8y99e7we98			 "
            + "yhfdsuyf892yr98ghwequifyh879esa6yf83g2ui1rfgtvbiygf			 "
            + "appcanuse923yhr32hjfklsuexsecuritykeyboardkdsahfuy			 "
            + "28973yr^%UBFG%^&*IO^bjk789234y6cxzv98324df96621378*			 "
            + "^&$69879872364327848e^$%^$*(&(&wrtf32fuiheappcanuse";
    // public static final String F_KEY =
    // "982398e4^$%^&%^&%^&%&^$#$#sdfsda90239%^)f8y99e7we98 "
    // + "yhfdsuyf892yr98ghwequifyh879esa6yf83g2ui1rfgtvbiygf "
    ...
    //
    // try {
    // int i = endStr.getBytes(EMMConsts.CHARSET_UTF8).length;
    // int k = inByte.length;
    // byte[] testByte = new byte[i];
    // System.arraycopy(inByte, k - i, testByte, 0, i);
    // String test = new String(testByte, EMMConsts.CHARSET_UTF8);
